...
class Cron_Job {
	...
	public static function get_next_cronjob_execution_time(): string {
		...
		if ( $next_onetimeoff_timestamp ) {
			...
			$text .= sprintf( '<br/>There is current work in process being called recursivelly, with %s shows in the queue to be scraped<br/>', count( Queue_To_Process::get_queued_shows() ) );
